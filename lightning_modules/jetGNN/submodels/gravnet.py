...
class GravNet(JetGNNBase):
    ...
    def output_step(self, x, batch, all_x = None):
        ...
        # Add dropout
        if "dropout" in self.hparams and self.hparams["dropout"] > 0.0:
            graph_level_inputs = F.dropout(graph_level_inputs, p=self.hparams["dropout"], training=self.training)
    ...
    def get_layer_structure(self):
        ...
        if "layer_shape" not in self.hparams or self.hparams["layer_shape"]=="flat":
            ...
            self.layer_structure = [ [self.hparams["hidden"] // i , self.hparams["hidden"] // (i+1)] for i in range(self.hparams["n_graph_iters"]) ]
        elif self.hparams["layer_shape"] == "antipyramid":
            self.layer_structure = [ [self.hparams["hidden"] // i , self.hparams["hidden"] // (i-1)] for i in range(self.hparams["n_graph_iters"], 0, -1) ]
